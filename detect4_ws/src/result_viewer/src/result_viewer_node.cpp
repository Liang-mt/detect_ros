...
ResultViewer::ResultViewer() : it_(nh_) {
    // 使用私有节点句柄读取参数
...
ResultViewer::~ResultViewer() {
    ...
    if (detection_sub_) {
        ...
        ROS_INFO("Detection subscriber shutdown");
    }
    
    // 关闭OpenCV窗口
    cv::destroyAllWindows();
    ROS_INFO("OpenCV windows closed");
    
    ROS_INFO("ResultViewer node stopped");
}
...
void ResultViewer::image_callback(const sensor_msgs::ImageConstPtr& msg) {
    try {
        std::lock_guard<std::mutex> lock(data_mutex_);
...
void ResultViewer::detection_callback(const vision_msgs::Detection2DArray::ConstPtr& msg) {
    std::lock_guard<std::mutex> lock(data_mutex_);
